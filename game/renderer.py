...
def render_panel(data: dict) -> bytes:
    ...
    # 底部提示
    draw.text((w // 2, h - 24), "骑砍英雄传", fill=TEXT_MUTED, font=font_small, anchor="mt")
...
def render_help(commands: list[tuple[str, str]], prefix: str = "修仙") -> bytes:
    ...
    draw.text((w // 2, 25), "骑砍英雄传 · 指令", fill=ACCENT_GOLD, font=font_title, anchor="mt")
...
def render_ranking(rankings: list[dict], total: int, online: int,
    ...
    draw.text((w // 2, 22), "骑士排行榜", fill=ACCENT_GOLD, font=font_title, anchor="mt")
    draw.text((30, 48), f"总骑士: {total}", fill=TEXT_MUTED, font=font_small)
...
def render_online(players: list[dict], count: int) -> bytes:
    ...
    draw.text((w // 2, 22), f"在线骑士（{count}人）", fill=ACCENT_GOLD, font=font_title, anchor="mt")

    y = 55
    if not players:
        draw.text((w // 2, y), "当前无骑士在线", fill=TEXT_MUTED, font=font_main, anchor="mt")
    else:
        for p in players:
...
def render_checkin(data: dict) -> bytes:
    ...
    # 底部提示
    draw.text((w // 2, h - 25), "骑砍英雄传", fill=TEXT_MUTED, font=font_small, anchor="mt")
...
def render_afk_result(data: dict) -> bytes:
    ...
    # 标题
    draw.text((w // 2, 35), "挂机训练结算", fill=ACCENT_GOLD, font=font_title, anchor="mt")

    # 时长
    duration = data.get("duration_min", 0)
    draw.text((w // 2, 75), f"训练时长：{duration}分钟", fill=TEXT_SECONDARY, font=font_main, anchor="mm")

    # 获得经验
    exp = data.get("exp_gained", 0)
    draw.text((w // 2, 110), f"获得经验：{exp}", fill=ACCENT_CYAN, font=font_main, anchor="mm")

    # 爵位提升
    if sub_ups > 0:
        realm_name = data.get("realm_name", "")
        draw.text((w // 2, 145), f"爵位提升！当前：{realm_name}", fill=ACCENT_GREEN, font=font_main, anchor="mm")
...
def render_adventure(data: dict) -> bytes:
    ...
    if outcome == "stones":
        ...
        draw.text((w // 2, y), f"获得第纳尔：{stones}", fill=ACCENT_GOLD, font=font_main, anchor="mt")
    elif outcome == "exp":
        exp = data.get("exp_gained", 0)
        draw.text((w // 2, y), f"获得经验：{exp}", fill=ACCENT_CYAN, font=font_main, anchor="mt")
    elif outcome == "pill":
        pill = data.get("pill_name", "")
        draw.text((w // 2, y), f"获得药剂：{pill}", fill=ACCENT_GREEN, font=font_main, anchor="mt")
        ...
        if realm_changed:
            old_r = data.get("old_realm", "")
            new_r = data.get("new_realm", "")
            draw.text((w // 2, y + 28), f"爵位跌落：{old_r} → {new_r}", fill=ACCENT_RED, font=font_main, anchor="mt")
    elif outcome == "death":
        draw.text((w // 2, y), "不幸战死沙场，经验尽失……", fill=ACCENT_RED, font=font_main, anchor="mt")
    ...
    # 底部
    draw.text((w // 2, h - 20), "骑砍英雄传", fill=TEXT_MUTED, font=font_desc, anchor="mt")
...
def render_scenes(scenes: list[dict]) -> bytes:
    ...
    draw.text((w // 2, 25), "征战场景一览", fill=ACCENT_GOLD, font=font_title, anchor="mt")
...
def render_market(
    ...
    if not listings:
        ...
        draw.text(
            (img_w // 2, 40), "集 市", fill=ACCENT_GOLD, font=font_title, anchor="mt",
        )
        draw.text(
            (img_w // 2, 90), "集市暂无商品", fill=TEXT_MUTED, font=font_name, anchor="mt",
        )
        buf = io.BytesIO()
        img.save(buf, format="PNG")
        return buf.getvalue()
    ...
    # 标题
    draw.text(
        (img_w // 2, 24), "集 市", fill=ACCENT_GOLD, font=font_title, anchor="mt",
    )
