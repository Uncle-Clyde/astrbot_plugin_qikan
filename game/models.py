"""游戏数据模型：Player、序列化/反序列化 - 骑马与砍杀风格。"""
...
class Player:
    ...
    sub_realm: int = 0  # 小等级（新兵~精英: 0-9, 领主~伯爵: 0-3）
    exp: int = 0
    hp: int = 100
    max_hp: int = 100
    attack: int = 10
    ...
    permanent_lingqi_bonus: int = 0    # 药剂提供的永久体力上限加成
    heart_method: str = "无"  # 被动技能ID（或 "无"）
    weapon: str = "无"       # 武器
    gongfa_1: str = "无"     # 战斗技能一
    ...
    heart_method_mastery: int = 0  # 被动技能修炼阶段 0=入门,1=熟悉,2=精通,3=大师
    heart_method_exp: int = 0      # 当前阶段已积累的技能经验
    heart_method_value: int = 0    # 预存技能值（用于降级后转化）
    gongfa_1_mastery: int = 0      # 战斗技能一修炼阶段
    gongfa_1_exp: int = 0          # 战斗技能一当前阶段经验
    gongfa_2_mastery: int = 0      # 战斗技能二修炼阶段
    gongfa_2_exp: int = 0          # 战斗技能二当前阶段经验
    gongfa_3_mastery: int = 0      # 战斗技能三修炼阶段
    gongfa_3_exp: int = 0          # 战斗技能三当前阶段经验
    ...
    afk_cultivate_start: float = 0.0  # 训练挂机开始时间戳
    afk_cultivate_end: float = 0.0    # 训练挂机预定结束时间戳
    last_adventure_time: float = 0.0  # 上次征战时间戳
    death_count: int = 0              # 战死次数
    active_buffs: list[dict] = field(default_factory=list)  # 药剂临时buff列表
    unified_msg_origin: Optional[str] = None
    password_hash: Optional[str] = None  # Web 登录密码哈希
    
    # 地图系统状态
    map_state: PlayerMapState = field(default_factory=PlayerMapState)
    
    # 进行中的任务
    active_quests: list[dict] = field(default_factory=list)  # [{"quest_id": ..., "progress": ...}]
    
    # 等级系统
    level: int = 1                    # 玩家等级 (1-100)
    unallocated_points: int = 0       # 可分配属性点
    ...
    def to_dict(self, include_sensitive: bool = False) -> dict:
        ...
        head_eq = EQUIPMENT_REGISTRY.get(self.head)
        body_eq = EQUIPMENT_REGISTRY.get(self.body)
        hands_eq = EQUIPMENT_REGISTRY.get(self.hands)
        legs_eq = EQUIPMENT_REGISTRY.get(self.legs)
        shoulders_eq = EQUIPMENT_REGISTRY.get(self.shoulders)
        ...
        d = {
            ...
            "map_state": {
                "current_location": self.map_state.current_location,
                "x": self.map_state.x,
                "y": self.map_state.y,
                "travel_progress": self.map_state.travel_progress,
                "travel_destination": self.map_state.travel_destination,
                "travel_time": self.map_state.travel_time,
                "reputation": dict(self.map_state.reputation),
                "daily_quests_available": self.map_state.daily_quests_available,
    ...
    def from_dict(cls, data: dict) -> Player:
        ...
        map_state = PlayerMapState(
            current_location=map_state_data.get("current_location", ""),
            x=map_state_data.get("x", 500.0),
            y=map_state_data.get("y", 500.0),
            travel_progress=map_state_data.get("travel_progress", 0.0),
            travel_destination=map_state_data.get("travel_destination", ""),
            travel_time=map_state_data.get("travel_time", 0.0),
            active_quests=map_state_data.get("active_quests", []),
            completed_quests=map_state_data.get("completed_quests", []),
            reputation=map_state_data.get("reputation", {}),
        ...
        return cls(
            ...
            head=data.get("head", "无"),
            body=data.get("body", "无"),
            hands=data.get("hands", "无"),
            legs=data.get("legs", "无"),
