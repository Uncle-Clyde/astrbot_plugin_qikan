...
LOW_HP_WARNING_TEXT = "你已重伤濒危，仅剩1点生命，建议立刻退出副本；若继续冒进，死亡风险会急剧上升。"
...
class DungeonManager:
        ...
        *,
        ...
        if death_prob > 0 and random.random() < death_prob:
            return await self._resolve_dungeon_death(
                player,
                session,
                death_message=death_message or f"你在{context}中伤重不治，不幸战死沙场……",
                talisman_message=(
                    talisman_message
                    or f"你在{context}中本该战死，幸有保命符替你挡下死劫。你拖着残躯退出副本，征战结束。"
                ),
        ...
        if realm_drop_rate > 0 and random.random() < realm_drop_rate:
            ...
            return await self._finalize_failure_exit(
                ...
                session,
                message=(
                    f"你在{context}中险死还生，经验清零，爵位跌落{max(1, actual_drop)}级："
                    f"{old_name} → {new_name}，征战结束。"
                ),
        ...
        if serious:
            message = (
                ...
                f"当前HP：{new_effective_hp}/{effective_max_hp}，征战结束。"
            )
        else:
            message = (
                f"你在{context}中负伤撤离，"
    ...
    async def start(self, player: Player) -> dict:
        ...
        session = DungeonSession(user_id=player.user_id, current_layer=0,
                                 status="exploring")
        session.message = f"你踏入了{LAYER_NAMES[0]}，准备开始征战……"
    ...
    async def _resolve_disaster(
        ...
        if roll <= DISASTER_OUTCOMES["hp_damage"]:
            ...
            if dmg > 0 and (effective_stats["hp"] <= 1 or session.fatal_on_next_damage):
                ...
                return await self._resolve_failure_outcome(
                    ...
                    threat_gap=0.9,
                    severity_bonus=12.0,
                    death_message="你本已命悬一线，又遭天灾重创，不幸战死沙场……",
                    talisman_message="天灾本该夺你性命，所幸保命符替你挡下一劫。你拖着残躯逃出副本，征战结束。",
                    extra_result={
                        "passed": True,
                        "danger": "disaster",
                    },
        ...
        return await self._resolve_failure_outcome(
            ...
            threat_gap=1.0,
            severity_bonus=18.0,
            death_message="天灾降临，你被卷入毁灭乱流，不幸战死沙场……",
            talisman_message="天灾降临之际，保命符替你扛下死劫。你侥幸逃出生天，征战结束。",
            extra_result={
                "passed": True,
                "danger": "disaster",
            },
    ...
    def _generate_enemy(self, player: Player, layer: int,
        ...
        # 敌人名称
        monster_names = ["野兽", "山贼", "流寇", "敌方士兵", "雇佣兵"]
        enemy_names = ["游侠", "亡命徒", "敌对骑士", "独行侠", "赏金猎人"]
    ...
    async def _finish_combat(self, player: Player,
        ...
        if outcome == "win":
            ...
            return await self._resolve_failure_outcome(
                ...
                severity_bonus=6.0,
                death_message=f"被{enemy_name}击败，伤势过重，不幸战死沙场……",
                talisman_message=f"被{enemy_name}击败之际，保命符替你扛下死劫。你重伤退出副本，征战结束。",
            )
            ...
            session.combat = None
            session.status = "failed"
            session.message = "逃离战斗，征战结束，保留已获奖励"
            self._mark_adventure_finished(player)
            await self._engine._save_player(player)
            result = {
                "success": True,
            ...
            session.combat = None
            session.status = "failed"
            session.message = "战斗超时，双方脱离，征战结束"
            self._mark_adventure_finished(player)
            await self._engine._save_player(player)
            result = {
                "success": True,
    ...
    async def resolve_pvp_result(self, pvp_session) -> dict | None:
        ...
        if pvp_session.winner_id is None:
            return await self._finalize_failure_exit(
                player,
                session,
                message="你与在线对手鏖战许久仍未分出胜负，征战被迫中止。",
                result_extra={
        ...
        return await self._resolve_failure_outcome(
            ...
            threat_gap=threat_gap,
            severity_bonus=10.0,
            death_message=f"你在遭遇战中被在线玩家{enemy_name}击溃，不幸战死沙场……",
            talisman_message=f"在线玩家{enemy_name}本可取你性命，所幸保命符替你挡下死劫。你重伤退出副本，征战结束。",
        )
