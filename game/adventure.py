...
async def _apply_victory_rewards(player: Player, result: dict, enemy_scale: float):
    ...
    if "gold" in reward_types:
        gold = _apply_stones(player, enemy_scale)
        result["stones_gained"] = gold
        reward_lines.append(f"金币 +{gold}")
    if "exp" in reward_types:
        exp = _apply_exp(player, enemy_scale)
        result["exp_gained"] = exp
        reward_lines.append(f"经验 +{exp}")
    if "potion" in reward_types:
        potion_name = await _apply_pill(player)
        result["pill_name"] = potion_name
        reward_lines.append(f"药水 +{potion_name}")
    if "weapon" in reward_types:
        equip = await _apply_equip_drop(player)
        if equip:
            result["equip_name"] = equip["name"]
            result["equip_tier"] = equip["tier_name"]
            reward_lines.append(f"装备 +{equip['tier_name']}级【{equip['name']}】")
        else:
            fallback_exp = _apply_exp(player, enemy_scale * 0.8)
            result["exp_gained"] = result.get("exp_gained", 0) + fallback_exp
            reward_lines.append(f"战利品池空，改为经验 +{fallback_exp}")

    skill_drop = await _apply_heart_method_drop(player, enemy_scale)
    if skill_drop:
        result["heart_method_drop"] = skill_drop
        reward_lines.append(
            f"技能书 +【{skill_drop['manual_name']}】（{skill_drop['realm_name']}·普通）"
        )
    ...
    if gf_drop:
        ...
        reward_lines.append(
            f"战技卷轴 +{gf_drop['tier_name']}级【{gf_drop['name']}】"
        )
    ...
    result["message"] = (
        f"击败了{result['enemy_realm_name']}级敌人，战利品组合：{combo_desc}\n"
        + "\n".join(reward_lines)
    )
...
def _apply_stones(player: Player, enemy_scale: float) -> int:
    ...
    gold = random.randint(int(50 * realm_mult * scale), int(220 * realm_mult * scale))
...
async def _apply_equip_drop(player: Player) -> dict | None:
    """获得装备 - 骑砍风格：仅掉落普通或精良装备。"""
    # 仅允许普通/精良
    tiers = [EquipmentTier.COMMON, EquipmentTier.FINE]
...
async def _apply_heart_method_drop(player: Player, enemy_scale: float) -> dict | None:
    """掉落被动技能书 - 骑砍风格。"""
    drop_rate = min(0.35, _SKILL_DROP_BASE_RATE + max(0.0, enemy_scale - 1.0) * 0.12)
    ...
    return {
        ...
        "name": hm.name,
        "realm": hm.realm,
        "realm_name": REALM_CONFIG.get(hm.realm, {}).get("name", "未知等级"),
        "quality": hm.quality,
    }
...
def _apply_injured(player: Player, result: dict, enemy_attack: int):
    ...
    result["message"] = (
        f"在【{cat}·{scene}】中不敌敌人，负伤{damage}点，"
...
def _apply_injured_realm_down(player: Player, result: dict, enemy_attack: int):
    ...
    result["message"] = (
        f"在【{cat}·{scene}】中血战逃生，负伤{damage}点，"
        f"等级下降{max(1, actual_drop)}级：{old_name} → {new_name}！"
    )
...
def _apply_gongfa_mastery(player: Player) -> list[str]:
    ...
    for slot in ("gongfa_1", "gongfa_2", "gongfa_3"):
        ...
        if cur_exp >= gf.mastery_exp:
            ...
            msgs.append(f"战技【{gf.name}】修炼至{new_level}！")
        setattr(player, exp_attr, cur_exp)
    return msgs
