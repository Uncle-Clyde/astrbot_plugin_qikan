...
DEFAULT_SUBMIT_POINTS: dict[str, int] = {
    ...
    "equipment_2": 120,   # 精品
    "equipment_3": 250,   # 极品
    "heart_method_0": 30,   # 普通技能
    "heart_method_1": 80,   # 精良技能
    "heart_method_2": 180,  # 稀有技能
    "gongfa_0": 20,   # 初级战技
    "gongfa_1": 50,   # 中级战技
    "gongfa_2": 120,  # 高级战技
    "gongfa_3": 250,  # 终极战技
...
async def create_sect(
    ...
    *,
    ...
    if len(name) < NAME_MIN_LEN or len(name) > NAME_MAX_LEN:
        return {"success": False, "message": f"家族名称需 {NAME_MIN_LEN}-{NAME_MAX_LEN} 个字"}

    if player.spirit_stones < CREATE_COST:
        return {"success": False, "message": f"第纳尔不足，创建家族需要 {CREATE_COST} 第纳尔"}

    existing = await dm.load_player_sect(player.user_id)
    if existing:
        return {"success": False, "message": "你已加入家族，无法创建新家族"}

    dup = await dm.load_sect_by_name(name)
    if dup:
        return {"success": False, "message": f"家族名「{name}」已被使用"}
    ...
    if name_reviewer:
        ...
        if not allow:
            ...
            return {"success": False, "message": f"家族名审核未通过：{reason}"}
    ...
    try:
        ...
        if existing:
            return {"success": False, "message": "你已加入家族，无法创建新家族"}
        return {"success": False, "message": "家族创建失败，请稍后重试"}

    player.spirit_stones -= CREATE_COST

    return {
        "success": True,
        "message": f"家族「{name}」创建成功！消耗 {CREATE_COST} 第纳尔",
        "sect_id": sect_id,
    }
...
async def disband_sect(player: "Player", dm: "DataManager") -> dict:
    ...
    if membership["role"] != "leader":
        return {"success": False, "message": "只有家主才能解散家族"}

    sect_id = membership["sect_id"]
    count = await dm.count_sect_members(sect_id)
    if count > 1:
        return {"success": False, "message": "家族内还有其他成员，请先移除所有成员或转让家主后再解散"}
    ...
    return {"success": True, "message": f"家族「{sect_name}」已解散"}
...
async def get_my_sect(user_id: str, dm: "DataManager") -> dict:
    ...
    for m in members:
        m["realm_name"] = get_realm_name(m.get("realm", 0), m.get("sub_realm", 0))
        m["role_name"] = ROLE_NAMES.get(m["role"], "成员")
    ...
    return {
        ...
        "my_role_name": ROLE_NAMES.get(membership["role"], "成员"),
...
async def join_sect(
    ...
    existing = await dm.load_player_sect(player.user_id)
    if existing:
        return {"success": False, "message": "你已加入家族，请先退出当前家族"}
    ...
    if sect["join_policy"] == "closed":
        return {"success": False, "message": "该家族暂不接受新成员"}

    if player.realm < sect["min_realm"]:
        min_name = get_realm_name(sect["min_realm"], 0)
        return {"success": False, "message": f"加入该家族需要至少达到「{min_name}」爵位"}

    count = await dm.count_sect_members(sect_id)
    if count >= sect["max_members"]:
        return {"success": False, "message": "家族成员已满"}
...
async def leave_sect(user_id: str, dm: "DataManager") -> dict:
    ...
    if not membership:
        return {"success": False, "message": "你尚未加入任何家族"}
    if membership["role"] == "leader":
        return {"success": False, "message": "家主不可直接退出，请先转让家主或解散家族"}
...
async def kick_member(
    ...
    if not tgt_mem or tgt_mem["sect_id"] != op_mem["sect_id"]:
        return {"success": False, "message": "目标不在你的家族中"}
    ...
    if op_mem["role"] == "leader":
        ...
        if tgt_rank <= 1:
            return {"success": False, "message": "副家主只能踢出长老和普通成员以下成员"}
        if tgt_rank <= 2:
            return {"success": False, "message": "副家主无法踢出长老"}
    else:
    ...
    return {"success": True, "message": f"已将「{tgt_name}」踢出家族"}
...
async def update_sect_info(
    ...
    if not op_mem:
        return {"success": False, "message": "你尚未加入任何家族"}
    if op_mem["role"] not in ("leader", "vice_leader"):
        return {"success": False, "message": "只有家主或副家主才能修改家族信息"}
    ...
    return {"success": True, "message": "家族信息已更新"}
...
async def transfer_leader(
    ...
    if not op_mem:
        return {"success": False, "message": "你尚未加入任何家族"}
    if op_mem["role"] != "leader":
        return {"success": False, "message": "只有家主才能转让家主"}
    ...
    if tgt_mem["role"] != "elder":
        ...
        if elder_count >= MAX_ELDERS:
            return {
                "success": False,
                "message": f"当前长老名额已满（{MAX_ELDERS}人），请先调整长老人数后再转让家主",
            }
    ...
    return {"success": True, "message": f"已将家主转让给「{tgt_name}」，你已变为长老"}
...
async def warehouse_deposit(
    ...
    *,
    ...
    if parse_stored_heart_method_item_id(item_id):
        return {"success": False, "message": "临时技能书无法上交到仓库"}
    ...
    if not result["success"]:
        ...
        if reason == "warehouse_full":
            return {"success": False, "message": "家族仓库已满"}
        if reason == "sect_not_found":
            return {"success": False, "message": "家族不存在"}
        if reason == "member_not_found":
            return {"success": False, "message": "你已不在该家族中"}
    ...
    if is_leader:
        return {
            "success": True,
            "message": f"成功存入「{name}」x{count}到家族仓库",
            "item_name": name,
            "count": count,
            "points_earned": 0,
...
async def warehouse_exchange(
    ...
    *,
    ...
    # 临时技能书绑定过期时间，禁止流转
    if parse_stored_heart_method_item_id(item_id):
        return {"success": False, "message": "临时技能书无法从仓库兑换"}
...
async def warehouse_list(user_id: str, dm: "DataManager") -> dict:
    ...
    return {
        ...
        "my_contribution": contribution,
        "my_role": role,
        "my_role_name": ROLE_NAMES.get(role, "成员"),
...
async def set_submit_rule(
    ...
    if not membership:
        return {"success": False, "message": "你尚未加入任何家族"}
    if membership["role"] != "leader":
        return {"success": False, "message": "只有家主才能设置贡献点规则"}
...
async def set_exchange_rule(
    ...
    *,
    ...
    if not membership:
        return {"success": False, "message": "你尚未加入任何家族"}
    if membership["role"] != "leader":
        return {"success": False, "message": "只有家主才能设置兑换规则"}
