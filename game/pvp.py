...
class PvPManager:
        ...
        *,
        ...
        if status == "pending":
            session.combat_log.append(
                f"刀剑出鞘！【{player_b.name}】需在{countdown_seconds}秒内迎战，逾期视为怯战！"
            )
    ...
    def activate_session(self, session_id: str) -> dict | None:
        ...
        if session.status == "pending":
            ...
            if not session.combat_log or session.combat_log[-1] != "决战开始！":
                session.combat_log.append("决战开始！")
    ...
    def respond_challenge(
        ...
        challenger_name = session.state_b.enemy_name or "挑战者"
        if session.countdown_deadline and time.time() >= session.countdown_deadline:
            return self._finish_pending_challenge(
                ...
                result_message=f"{player.name}未在10秒内迎战，{challenger_name}不战而胜！",
            )
        ...
        if not accept:
            return self._finish_pending_challenge(
                ...
                result_message=f"{player.name}怯战退缩，{challenger_name}大获全胜！",
            )

        payload = self.activate_session(session_id)
        if not payload:
            return {"success": False, "message": "对决启动失败，请稍后重试"}
        payload["started"] = True
        payload["message"] = "已接受挑战，决战开始！"
    ...
    def expire_challenge(self, session_id: str) -> dict | None:
        ...
        if session.countdown_deadline and time.time() < session.countdown_deadline:
            return None
        challenger_name = session.state_b.enemy_name or "挑战者"
        defender_name = session.state_a.enemy_name or "守擂者"
        return self._finish_pending_challenge(
            session,
            end_reason="challenge_timeout",
            result_message=f"{defender_name}未在10秒内迎战，{challenger_name}不战而胜！",
        )
    ...
    async def submit_flee_request(
        ...
        if not player:
            return {"success": False, "message": "角色不存在"}
        if session.status == "pending":
            return {"success": False, "message": "战斗尚未开始，不能提前投降"}
        if session.status != "waiting":
            return {"success": False, "message": "当前回合正在结算，请稍后"}
        if session.flee_offer:
            return {"success": False, "message": "已有投降请求待处理"}
        if user_id not in (session.player_a_id, session.player_b_id):
            return {"success": False, "message": "你不在这场对战中"}

        if user_id == session.player_a_id and session.action_a is not None:
            return {"success": False, "message": "本回合已出招，无法再申请投降"}
        ...
        session.combat_log.append(
            f"【{player.name}】请求投降，愿献上{self._format_offer_items(offer['items'])}"
            f"（合计{offer['total_value']}金币）"
        )
        return {
            "success": True,
            "message": (
                f"投降请求已发出，当前赔款价值{offer['total_value']}，"
                "尚未达到1000金币，需对方同意"
            ),
            "pvp_state_a": session.to_dict(session.player_a_id),
            "pvp_state_b": session.to_dict(session.player_b_id),
    ...
    async def respond_flee_request(
        ...
        if not player:
            return {"success": False, "message": "角色不存在"}
        if session.status == "pending":
            return {"success": False, "message": "战斗尚未开始"}
        if session.status != "waiting":
            return {"success": False, "message": "当前回合正在结算，请稍后"}
        if not session.flee_offer:
            return {"success": False, "message": "当前没有待处理的投降请求"}

        requester_id = str(session.flee_offer.get("requester_id", ""))
        if user_id == requester_id:
            return {"success": False, "message": "不能回应自己发出的投降请求"}
        if user_id not in (session.player_a_id, session.player_b_id):
            return {"success": False, "message": "你不在这场对战中"}
        ...
        session.flee_offer = None
        session.combat_log.append(f"【{player.name}】拒绝了【{requester_name}】的投降请求")
        return {
            "success": True,
            "message": "你拒绝了对方的投降请求，战斗继续",
            "pvp_state_a": session.to_dict(session.player_a_id),
            "pvp_state_b": session.to_dict(session.player_b_id),
            "ended": False,
            "state_changed": True,
    ...
    async def _resolve_round(self, session: PvPSession) -> dict:
        ...
        if knocked_out_a or knocked_out_b:
            ...
            if knocked_out_a and knocked_out_b:
                ...
                session.result_message = f"【{player_b.name}】斩敌获胜！"
            else:
                session.winner_id = session.player_a_id
                session.result_message = f"【{player_a.name}】斩敌获胜！"
            ...
            if knocked_out_a:
                ...
                if extra_a.get("death_prevented"):
                    session.combat_log.append(f"【{player_a.name}】触发保命符，侥幸生还！")
                elif extra_a.get("defeat_survived"):
                    session.combat_log.append(f"【{player_a.name}】重伤倒地，被俘虏！")
                elif extra_a.get("died"):
                    session.combat_log.append(f"【{player_a.name}】战死沙场！")
            if knocked_out_b:
                ...
                if extra_b.get("death_prevented"):
                    session.combat_log.append(f"【{player_b.name}】触发保命符，侥幸生还！")
                elif extra_b.get("defeat_survived"):
                    session.combat_log.append(f"【{player_b.name}】重伤倒地，被俘虏！")
                elif extra_b.get("died"):
                    session.combat_log.append(f"【{player_b.name}】战死沙场！")
            ...
            if session.state_a.player_hp > session.state_b.player_hp:
                session.winner_id = session.player_a_id
                session.result_message = f"体力耗尽，【{player_a.name}】以生命优势获胜！"
            elif session.state_b.player_hp > session.state_a.player_hp:
                session.winner_id = session.player_b_id
                session.result_message = f"体力耗尽，【{player_b.name}】以生命优势获胜！"
            else:
                session.winner_id = None
                session.result_message = "体力耗尽，双方筋疲力尽，平局！"
            session.combat_log.append(session.result_message)
    ...
    def _normalize_action(
        ...
        # 骑砍风格支持的动作
        valid_actions = {
            "attack", "defend", "gongfa",
            "charge", "feint", "battle_cry", "berserk"
        }
        if raw_action not in valid_actions:
            return {"success": False, "message": f"未知动作: {raw_action}，可用：攻击/防御/冲锋/虚晃/战吼/狂暴"}
        
        # 检查冲锋/狂暴等需要足够体力
        stamina_cost = {
            "charge": 20, "feint": 15, "battle_cry": 25, "berserk": 30
        }
        if raw_action in stamina_cost:
            my_state = session.state_a if user_id == session.player_a_id else session.state_b
            cost = stamina_cost[raw_action]
            if my_state.player_lingqi < cost:
                return {"success": False, "message": f"体力不足，需要{cost}点，当前{my_state.player_lingqi}点"}
        ...
        if gongfa_slot not in {"gongfa_1", "gongfa_2", "gongfa_3"}:
            return {"success": False, "message": "请选择要使用的战技槽位"}

        gongfa_id = getattr(player, gongfa_slot, "无")
        if not gongfa_id or gongfa_id == "无":
            return {"success": False, "message": "该槽位没有装备战技"}

        gongfa = GONGFA_REGISTRY.get(gongfa_id)
        if not gongfa:
            return {"success": False, "message": "战技数据异常"}
        ...
        if not is_regen and my_state.player_lingqi < gongfa.lingqi_cost:
            return {
                "success": False,
                "message": f"体力不足，需要{gongfa.lingqi_cost}，当前{my_state.player_lingqi}",
            }
        return {
            "success": True,
    ...
    def _build_action_plan(self, state: CombatState, player: Player, action: dict) -> dict:
        ...
        if raw_action == "charge":
            ...
            return {
                "attack_power": state.player_attack + charge_bonus,
                "defending": False,
                "heal": 0,
                "lingqi_regen": 0,
        ...
        if raw_action == "berserk":
            ...
            return {
                "attack_power": berserk_attack,
                "defending": False,
                "heal": 0,
                "lingqi_regen": 0,
                "summary": "狂暴姿态！攻击翻倍，无防御",
            }
        
        if raw_action == "gongfa":
            return self._handle_gongfa_action(state, player, action)
        ...
        return {
            ...
            "defending": False,
            "heal": 0,
            "lingqi_regen": int(state.player_max_lingqi * 0.08),
            "summary": "挥剑攻击（体力+8%）",
        }

    def _handle_gongfa_action(self, state: CombatState, player: Player, action: dict) -> dict:
        """处理战技/技能 - 骑砍风格。"""
        ...
        stamina_regen = int(bonus["lingqi_regen"] * 2) if bonus["lingqi_regen"] > 0 else 0
        defending = bonus["defense_bonus"] > 0
        cost_msg = "免费" if is_regen else f"消耗{gongfa.lingqi_cost}体力"
        return {
            "attack_power": attack_power,
            "defending": defending,
            "heal": heal,
            "lingqi_regen": stamina_regen,
            "summary": f"施展战技【{gongfa.name}】({cost_msg})",
        }
    ...
    def _normalize_flee_items(self, player: Player, offer_items: list[dict] | None) -> dict:
        ...
        if not isinstance(offer_items, list) or not offer_items:
            return {"success": False, "message": "投降时必须上交物品作为赔款"}

        merged: dict[str, int] = {}
        for entry in offer_items:
            if not isinstance(entry, dict):
                return {"success": False, "message": "赔款物品格式错误"}
        ...
        for item_id, count in merged.items():
            ...
            if price is None:
                return {"success": False, "message": f"【{item_id}】不可作为赔款"}
        ...
        if total_value <= 0:
            return {"success": False, "message": "赔款价值必须大于 0"}
        return {
            "success": True,
            "offer": {
        ...
        *,
        ...
        if auto_approved:
            session.result_message = (
                f"【{requester.name}】投降并献上{offer_summary}，"
                f"【{opponent.name}】获胜！"
            )
        else:
            session.result_message = (
                f"【{opponent.name}】接受了【{requester.name}】的投降，"
                f"【{requester.name}】献上{offer_summary}后被释放。"
            )
        session.combat_log.append(session.result_message)
        await self._sync_players(player_a, player_b, session)
        return self._build_resolve_payload(
