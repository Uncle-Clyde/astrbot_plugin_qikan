...
async def list_item(
    ...
    if parse_stored_heart_method_item_id(item_id):
        return {"success": False, "message": "临时技能书不可上架"}
    ...
    if unit_price < MIN_UNIT_PRICE:
        return {"success": False, "message": f"单价不能低于 {MIN_UNIT_PRICE} 第纳尔"}
    ...
    if player.spirit_stones < fee:
        return {
            "success": False,
            "message": f"第纳尔不足以支付手续费（需 {fee} 第纳尔，当前 {player.spirit_stones}）",
        }
    ...
    return {
        ...
        "message": (
            ...
            f"单价 {unit_price} 第纳尔，总价 {total_price} 第纳尔，"
            f"手续费 {fee} 第纳尔（{rate*100:.1f}%），"
            f"编号 {listing_id[:6]}"
        ),
...
async def buy_item(
    ...
    if buyer.spirit_stones < total_price:
        return {
            "success": False,
            "message": f"第纳尔不足（需 {total_price}，当前 {buyer.spirit_stones}）",
        }
    ...
    try:
        async with dm.transaction() as tx:
            ...
            if cur.rowcount <= 0:
                raise DataManager.TransactionAbort("该商品已被其他骑士抢先购买")
        ...
        logger.error(f"集市交易数据库操作失败: {e}")
        return {"success": False, "message": "交易失败，请稍后重试"}

    return {
        "success": True,
        "message": f"成功购买「{item_name}」x{listing['quantity']}，花费 {total_price} 第纳尔",
